...
Before running through the questions (and answers) let's load the R packages used in this tutorial. Only the most recent Gapminder data (2007) will be used.
...
If you have multiple scales `gggvis` will unhelpfully superimpose them in the legend. In the plot below `size` is mapped to (pop) and `fill` to (continent).
